...
export function LifecycleControl({leadId,status,projectId}:{leadId:string;status:LeadStatus;projectId?:string}){
 ...
 const [messages,setMessages]=useState<InboundMessage[]>([]); const [error,setError]=useState(""); const [pending,setPending]=useState(false);
 // Gewone async handler i.p.v. startTransition: React 19 levert een rejection van een
 // async transition-callback af aan de error boundary i.p.v. lokale code.
 const submit=async()=>{
  if(!next)return;
  setError("");
  setPending(true);
  try{
   const result=await transitionLeadAction({leadId,expected:status,next,reason,projectId,messageId:messageId||null});
   if(result.error)setError(result.error);
   else router.refresh();
  }catch(e){
   setError(e instanceof Error?e.message:"Statusupdate mislukt");
  }finally{
   setPending(false);
  }
 };
 ...
 return <section className="space-y-3">
  ...
  {options.length>0&&<form className="space-y-3" onSubmit={e=>{e.preventDefault();submit();}}>
