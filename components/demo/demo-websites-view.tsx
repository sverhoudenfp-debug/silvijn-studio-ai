...
import { demoStatusMeta } from "@/lib/mock-data";
...
export function DemoWebsitesView({ rows: allRows }: { rows: DemoLeadRow[] }) {
  ...
  const industries = useMemo(
    () => Array.from(new Set(allRows.map(({ demo }) => demo.industry))).sort(),
    [allRows]
  );
  const cities = useMemo(
    () => Array.from(new Set(allRows.map(({ demo }) => demo.city))).sort(),
    [allRows]
  );
  ...
  const summary = useMemo(
    () => ({
      ...
      ready: allRows.filter(({ demo }) => demo.status === "ready").length,
      generating: allRows.filter(({ demo }) => demo.status === "generating").length,
      failed: allRows.filter(({ demo }) => demo.status === "failed").length,
    }),
    [allRows]
  );
  ...
  const rows = useMemo(() => {
    ...
    const filtered = allRows.filter(({ demo }) => {
      if (
        q &&
  ...
  }, [allRows, query, status, template, industry, city, sort]);
