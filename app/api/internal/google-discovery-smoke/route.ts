...
export async function POST(request: Request) {
  ...
  let rawGoogleResultCount: number | null = null;
  const diagnosticFetcher: typeof fetch = async (input, init) => {
    const response = await fetch(input, init);
    if (response.ok) {
      try {
        const parsed = await response.clone().json() as { places?: unknown };
        rawGoogleResultCount = Array.isArray(parsed.places) ? parsed.places.length : 0;
      } catch {}
    } else {
      try {
    ...
    return NextResponse.json(
      {
        ...
        rawGoogleResultCount,
        candidateCount: page.candidates.length,
        mappingSucceeded: rawGoogleResultCount === page.candidates.length,
