 ...
 *
 * Providers: NextJsWebsiteGenerator (geïmplementeerd) en ShopifyWebsiteGenerator
 * (stub voor latere fase) — de service kiest via deze interface.
 */
...
/**
 * Shopify-provider — bewuste stub (Fase 9): provider-based architecture zodat
 * latere fases een Shopify-theme-generator kunnen toevoegen zonder de core
 * generator of de Next.js-provider aan te passen.
 */
export class ShopifyWebsiteGenerator implements WebsiteGeneratorProvider {
  readonly framework = "shopify" as const;

  generate(): WebsiteGeneratorResult {
    throw new WebsiteGenerationNotSupportedError(
      "De Shopify-websitegenerator is nog niet geïmplementeerd (latere fase). De Next.js-generator is beschikbaar."
    );
  }
}
